...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('cookies')
		.setDescription('Gibt dir Keeeeekseeee')
		.addIntegerOption(opt =>
			opt.setName('count').setDescription('Wie viele Kekese du haben willst').setRequired(true)
		),
	name: this.data.name,
	...
	async execute(ita, args, client) {
		...
		if (maxguild == 0)
			return embeds.error(
				ita,
				'Limit erreicht',
				'Dieser Server ist hat sein zugewiesenes Limit diese Minute bereits erreicht.\nFüge mehr Kekse mit `/addserver` hinzu, um das Limit zu erhöhen.',
				true
			);
		if (maxuser == 0)
			return embeds.error(
				ita,
				'Limit erreicht',
				'Du hast dein zugewiesenes Limit diese Minute bereits erreicht.\nBitte warte einen Moment, bevor du mehr Kekse isst.',
				true
			);
		if (args.count == 0)
			return embeds.success(
				ita,
				'Keine Kekse ausgeliefert',
				'Herzlichen Glückwunsch, du hast keine Kekse bekommen.',
				true
			);
		...
		let embed = new discord.MessageEmbed()
			...
			.setDescription(
				`${args.count} Kekse wurden in deinem Lager zwischengespeichert.\nDu hast aktuell ${user.data
					.cookies} Kekse.`
			);
