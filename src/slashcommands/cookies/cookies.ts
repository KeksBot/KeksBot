...
const options: CommandOptions = {
    ...
    async execute(ita, args, client) {
        ...
        let maxuser = 
            user.storage.data.badges & 0b1 ? 512 :      // Team
            user.storage.data.badges & 0b10 ? 512 :     // VIP
            user.storage.data.badges & 0b100 ? 256 :    // Verified (?)
            user.storage.data.badges & 0b1000 ? 256 :   // Partner
            128
        var maxguild = 2 ** (guild.storage.data.level + 8)
        if(guild.storage.data.partner && guild.storage.data.partner == 1 && maxguild < 65536) maxguild = 65536
        if(guild.storage.data.verified && maxguild < 4194304) maxguild = 4194304
        ...
        ita.user.setData({ cookies: Math.floor(user.storage.data.cookies) })
