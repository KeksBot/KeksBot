...
const validatePermissions = command => {
	...
	if (!validPermissions.includes(command.permission))
		throw new Error(`Unbekannte Permission "${command.permission} bei "${command.name}"`);
};
...
module.exports = async client => {
	...
	client.on('interactionCreate', async function(ita) {
		...
		if (ita.options.getSubcommandGroup(false) || false)
			args.subcommandgroup = ita.options.getSubcommandGroup(false);
		...
		getData('userdata', ita.user.id).then(async function(data) {
			...
			if (data.banned && data.banned.time) {
				...
				if (!data.banned.time || data.banned.time < Date.now()) {
					...
					let timestamp = '';
					if (data.banned.time && data.banned.time != Infinity)
					...
					embeds.error(
						ita,
						'Nutzung verboten',
						`Du wurdest von der KeksBot Nutzung gebannt.\n${reason}\n\nSolltest du Fragen zu diesem Fall haben, wende dich bitte an das [KeksBot Team](discord.gg/g8AkYzWRCK).${timestamp}`,
						true
					);
				}
				if (data.banned.time && data.banned.time < Date.now()) {
		...
		if (ita.user.data == -1 || ita.guild.data == -1)
			return embeds
				.error(
					ita,
					'Fehler',
					'Timeout der beim Laden erforderlichen Daten. Bitte probiere es später erneut.',
					true
				)
				.catch();
		...
		if (timestamps.has(ita.user.id)) {
			...
			if (now < expirationTime) {
				...
				return embeds.error(
					ita,
					'Cooldown',
					`Du kannst den ${command.name} Befehl erst wieder ${timeLeft} benutzen.`,
					true
				);
			}
		}
		...
		try {
			...
			console.log(
				`${d.getDate()}.${d.getMonth() +
					1}.${d.getFullYear()} ${d.getHours()}:${d.getMinutes()}:${d.getSeconds()} | ${ita.user
					.tag} | ID: ${ita.user.id} | ${ita.guild.name} | ID: ${ita.guild
					.id} | ${command.name} | [ ${argsarray.join(', ')} ]`
			);
			...
			return embeds.error(
				ita,
				'Fehler',
				'Beim Ausführen des Commands ist ein unbekannter Fehler aufgetreten.\nBitte probiere es später erneut.',
				true,
				true
			);
		}
